...
# Prefer local repo when script is run from a clone (avoids remote tag resolution
# issues, proxy errors on the Git URL, and missing-tag failures on GitLab).
$repoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
$localPyproject = Join-Path $repoRoot "pyproject.toml"

$sourceSpec = if (Test-Path $localPyproject) {
	Write-Info "Detected local repo clone at: $repoRoot — installing from local source."
	$repoRoot
}
elseif ($Latest -or [string]::IsNullOrWhiteSpace($Version)) {
	"git+$RepoUrl"
}
else {
	"git+$RepoUrl@$Version"
...
if (-not (Test-Path $localPyproject)) {
...
try {
	...
	if ($installError -match "pypi\.org|failed to fetch|proxy|timed out|timeout|name or service not known|could not resolve|connection|git operation failed|failed to fetch into") {
