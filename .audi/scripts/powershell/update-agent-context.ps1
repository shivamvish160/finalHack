...
$CURSOR_FILE   = Join-Path $REPO_ROOT '.cursor/rules/audi-rules.mdc'
$QWEN_FILE     = Join-Path $REPO_ROOT 'QWEN.md'
$AGENTS_FILE   = Join-Path $REPO_ROOT 'AGENTS.md'
$WINDSURF_FILE = Join-Path $REPO_ROOT '.windsurf/rules/audi-rules.md'
$JUNIE_FILE = Join-Path $REPO_ROOT '.junie/AGENTS.md'
$KILOCODE_FILE = Join-Path $REPO_ROOT '.kilocode/rules/audi-rules.md'
$AUGGIE_FILE   = Join-Path $REPO_ROOT '.augment/rules/audi-rules.md'
$ROO_FILE      = Join-Path $REPO_ROOT '.roo/rules/audi-rules.md'
...
$AGY_FILE      = Join-Path $REPO_ROOT '.agent/rules/audi-rules.md'
$BOB_FILE      = Join-Path $REPO_ROOT 'AGENTS.md'
$VIBE_FILE     = Join-Path $REPO_ROOT '.vibe/agents/audi-agents.md'
$KIMI_FILE     = Join-Path $REPO_ROOT 'KIMI.md'
$TRAE_FILE     = Join-Path $REPO_ROOT '.trae/rules/AGENTS.md'
$IFLOW_FILE    = Join-Path $REPO_ROOT 'IFLOW.md'

$TEMPLATE_FILE = Join-Path $REPO_ROOT '.audi/templates/agent-file-template.md'
...
function Validate-Environment {
    if (-not $CURRENT_BRANCH) {
        ...
        if ($HAS_GIT) { Write-Info "Make sure you're on a feature branch" } else { Write-Info 'Set AUDI_FEATURE environment variable or create a feature first' }
        exit 1
    }
    if (-not (Test-Path $NEW_PLAN)) {
        Write-Err "No plan.md found at $NEW_PLAN"
        Write-Info 'Ensure you are working on a feature with a corresponding spec directory'
        if (-not $HAS_GIT) { Write-Info 'Use: $env:AUDI_FEATURE=your-feature-name or create a new feature first' }
        exit 1
    }
    if (-not (Test-Path $TEMPLATE_FILE)) {
        Write-Err "Template file not found at $TEMPLATE_FILE"
        Write-Info 'Run audi init to scaffold .audi/templates, or add agent-file-template.md there.'
        exit 1
    }
}
