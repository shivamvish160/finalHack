...
def test_match_above_threshold_includes_similarity_and_actions():
    result = build_match_response(
        matches=[{"runbook_id": "rb-1", "title": "Restart checkout pods", "distance": 0.18, "recommended_actions": "kubectl rollout restart"}],
        threshold=0.72,
    )
...
def test_low_similarity_match_sets_below_threshold():
    ...
    result = build_match_response(
        matches=[{"runbook_id": "rb-2", "title": "Unrelated", "distance": 0.9, "recommended_actions": "n/a"}],
        threshold=0.72,
    )
    assert result["belowThreshold"] is True
